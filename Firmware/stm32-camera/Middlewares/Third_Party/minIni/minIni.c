 ...
 * - A string starting with 'y' or 'Y' i.e. YES
 * - A string starting with 't' or 'T' i.e. TRUE
 * - A string starting with 'e' or 'E' i.e. ENABLED
 * - A string starting with '1'
 *
 * A false boolean is found if one of the following is matched:
 * - A string starting with 'n' or 'N' i.e. NO
 * - A string starting with 'f' or 'F' i.e. FALSE
 * - A string starting with 'd' or 'D' i.e  DISABLED
...
int ini_getbool(const TCHAR *Section, const TCHAR *Key, int DefValue, const TCHAR *Filename)
{
  ...
  LocalBuffer[0] = (TCHAR)toupper((int)LocalBuffer[0]);
  if( LocalBuffer[0] == 'Y' || LocalBuffer[0] == '1' || LocalBuffer[0] == 'T' || LocalBuffer[0] == 'E' )
    ret = 1;
  else if (LocalBuffer[0] == 'N' || LocalBuffer[0] == '0' || LocalBuffer[0] == 'F' || LocalBuffer[0] == 'D')
    ret = 0;
  else
    ret = DefValue;
