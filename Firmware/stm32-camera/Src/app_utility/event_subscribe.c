 ...
 *
 * @copyright (c) 2013-2016 Applidyne Pty. Ltd. - All rights reserved.
 */
...
PUBLIC void
eventSubscribeInit( EventSubscribers subscribersList[],
    ...
    if( e ) // Silently ignore NULL events if asserts are not used.
    {
        ...
        if( eventSubscribers != 0 ) // any subscribers?
        {
            ...
            while( eventSubscribers > 0 )
            {
                ...
                ASSERT( mainTaskTable[p] );     // check if task is active
                                            // check queue can take event
                if( stateTaskPostFIFO( mainTaskTable[p], e ) )
                {
