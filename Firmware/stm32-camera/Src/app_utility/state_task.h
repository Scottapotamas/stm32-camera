...
#ifndef TASK_MAX
  #define TASK_MAX        16     //!< Maximum number of tasks supported
#endif
...
PUBLIC void
stateTaskCtor( StateTask  *me, State initialState );

/* -------------------------------------------------------------------------- */

//! Create the internal task structure and its event queues
PUBLIC StateTask *
stateTaskCreate( StateTask    *me,
                 StateEvent   *eventQueueData[],
                 uint8_t      eventQueueSize,
                 StateEvent   *requestQueueData[],
                 uint8_t      requestQueueSize );

/* -------------------------------------------------------------------------- */

//! table with tasks to execute
extern StateTask *mainTaskTable[TASK_MAX];
